...
def _session_agents_content(*, session: str, branch: str, config: CoconutConfig) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"You are working in Coconut session `{session}` on branch `{branch}`.",
            ...
            "",
            "Work normally in this managed worktree. When your local work should be integrated, run:",
            "",
            f"    coconut ready {session}",
            "",
            "Wait for Coconut to print a task file path in this Codex terminal. Read that task file,",
            "treat the current worktree as latest `main`, and re-implement or semantically merge the",
            "snapshot described by the task on top of latest `main`.",
            "",
            "After committing the final candidate and making sure the worktree is clean, run:",
            "",
            f"    coconut done {session}",
            "",
            "If the integration cannot be completed safely, run:",
            "",
            f"    coconut block {session} \"<reason>\"",
            "",
            "Use `coconut status` and `coconut log` to inspect Coconut state.",
            "",
