...
- 当 dirty session 所基于的 `main` 已经落后时放入 integration queue；
...
如果 Alice 有本地修改或本地 commit，而 `main` 从 Alice 上次同步后已经前进，`cocodex sync` 会请求进入 integration queue。轮到 Alice 时，Cocodex 会：

1. freeze Alice 的 session；
2. snapshot Alice 当前工作；
3. 将 Alice 的 worktree 重置到最新 `main`；
4. 在 `.cocodex/tasks/` 下写出 task file；
5. 如果 tmux 可用，会把 sync prompt 粘贴进 Alice 的 Codex 终端，并始终打印 task file 路径。
...
如果从 Alice 上次同步之后没有其他人推进 `main`，Cocodex 会直接发布 Alice 当前 worktree。如果 `main` 已经前进，Cocodex 会给 Alice 的 Codex 一个 task；Alice 的 Codex 在最新 `main` 上实现 feature A，提交后再次执行：
