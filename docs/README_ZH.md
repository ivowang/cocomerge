...
如果配置了 remote，每次 `coconut sync` 还会尝试把这台 server 上的本地 branch refs 强制同步到 remote。Coconut 以运行 daemon 的 server 上的 repo 状态为准，remote 分支不一致时可以被覆盖。远程同步是 best-effort：网络或认证失败只会打印 warning，不会中断本地开发，并会在后续 `coconut sync` 中重试。
...
如果希望 Coconut 为 server 上的本地分支保留远端副本，初始化前先添加 remote：
