...
如果配置了 remote，每次本地成功的 `cocodex sync` 还会尝试把本地 `main`、当前 session branch 和 Cocodex recovery refs 强制同步到 remote。它不会 push 或 prune 其他开发者的 branch。远程同步是 best-effort：网络或认证失败只会打印 warning，不会中断本地开发，并会在后续成功的 `cocodex sync` 中重试。
...
Operator 清理废弃旧 session 时使用：

```bash
cocodex delete alice
```

没有手动恢复命令。`delete` 不是卡住的 sync 的恢复入口，只用于团队已经确认
可以废弃的旧 session。如果某次 sync 无法继续，先用 `cocodex status` 或
`cocodex log` 查看状态，然后让提示中指出的 owner 开发者在自己的 managed
worktree 中运行 `cocodex sync`；如果该 Codex session 已关闭，先运行
`cocodex join <name>` 重新进入。
...
正常 semantic sync task 要求 `join` 在 tmux 中运行，这样 Cocodex 才能把
prompt 粘贴进 Codex。Cocodex 不会替开发者按 Enter；当 prompt 出现在 Codex
输入框里时，开发者需要先看一下，然后自己按 Enter 启动这个 task。如果没有可用
tmux target，Cocodex 会拒绝 task 启动，并恢复 session snapshot 以便重试。

生成的 `AGENTS.md` 会告诉 Codex 它处在 Cocodex 管理的协作 session 中，并说明正常同步只需要在 managed worktree 中运行 `cocodex sync`。

## 删除旧 Session

当某个开发者确认不再需要旧 session 时，operator 可以清理它的本地 Cocodex
注册和 managed 资源：
...
`delete` 会拒绝仍然 connected 的 session、持有 integration lock 的 session、
有 active sync task 的 session、存在未完成 Git 操作的 worktree、目标 branch
已被其他 worktree checkout 的情况，以及包含除 Cocodex 生成的 `AGENTS.md`
之外的 ignored files 的 worktree。这样可以避免把删除命令变成 sync 恢复手段，
也避免误删仍有价值的本地文件。

真正删除前，Cocodex 会在 `.cocodex/deleted/` 写 manifest，并在
`refs/cocodex/deleted/...` 下创建恢复 ref。如果 worktree 里有 tracked 或
untracked 改动，Cocodex 会先把它们保存到 dirty backup ref。随后它会移除本地
worktree，删除本地 `cocodex/<name>` 分支，删除 state 里的 session 和 queue
记录，并写入 `session_deleted` event。

`.cocodex/config.json` 里的 developer entry 会保留。只有当这个开发者以后也
不应该再 `cocodex join <name>` 时，才手动删除配置项。如果配置了 remote，
`delete` 还会 best-effort push deleted-session backup refs，并尝试删除远端
`cocodex/<name>` 分支；远端失败只打印 warning，不会回滚本地清理。
...
- 如果中断的 task 可以安全恢复，会保持或回到 `fusing`；
- 如果 task 启动尚未安全到达 Codex，会在可能时恢复 snapshot 后归一化；
...
如果 Git 出现 conflict、留下 unsafe state，或者轻量检查失败，Cocodex 会将 Alice 的 worktree 重置到最新 `main`，在 `.cocodex/tasks/` 下写出 task file，并把 sync prompt 粘贴进 Alice 的 Codex 输入框。prompt 成功投递到 pane 后，这次 semantic task 就被 Cocodex 接受；但 Cocodex 不会自动提交这个 prompt。Alice 需要在 Codex 里按 Enter 启动 task。如果无法安全投递 prompt，Cocodex 会把 Alice 的 worktree 恢复到 snapshot，释放 lock，并拒绝这次 `sync`。Alice 的 Codex 再读取 task file，在最新 `main` 上重新实现或语义融合 Alice 的 feature。目标 candidate 是最新 `main` 和 Alice snapshot 的行为并集。如果 Codex 发现两边存在真正的矛盾，例如互斥的产品行为、API contract、schema 或数据不变量，它必须询问用户如何处理，而不能静默丢掉其中一边。如果这个 task 到来时 Codex 正在处理另一个开发请求，Codex 应该先选择安全暂停点，保留当前请求剩余意图，完成这个 sync task，并在 sync 成功后继续之前暂停的开发工作。
...
- remote sync 只会在本地 sync 成功后 force-push 本地 `main`、当前 session branch 和 Cocodex recovery refs；
...
- active task 发布被拒绝时不会进入持久 `blocked` 状态；task 保持 active，同一个 session 修好问题后继续运行 `sync`；
- active task 发布要求 task file、snapshot ref、base ref、validation report 和 candidate commit 都仍然完整；
- 启动清理会在清理中断 task state 前创建 backup ref；
- remote sync 失败不会阻塞本地进度；Cocodex 会打印 warning，并在下一次 `sync` 时重试；
- 中断的 active task 会在 `cocodex join <name>` 时重新提示；旧版本遗留的 recovery 状态会被归一化，而不是成为正常流程。

## 拒绝与恢复

先运行 `cocodex status`。它会显示 daemon/session 版本、main guard 状态、每个 session 的 state、active task、branch head、当前配置的 remote，以及 integration lock 是否被持有。如果某个 session 有 active task，`status` 也会显示 task file、validation file、snapshot ref、base ref 和下一步动作。

当 Cocodex 命令失败时，不要立刻手动运行 Git recovery 命令。先保持相关 worktree 原样，按下面顺序处理：

1. 先读失败输出。新版 Cocodex 会打印 `Cocodex sync refused`，里面给出下一步安全动作。
2. 在项目仓库中运行 `cocodex status`，确认受影响的 session、state、active task、lock owner，以及是否有版本不一致。
3. 如果 session 有 active task，阅读 `status` 或拒绝输出里给出的 task file 和 validation file。
4. 优先让同一个开发者 session 继续处理。多数拒绝只需要提交 candidate、清理 worktree、补 validation report、重启 `cocodex join <name>`，或等当前 lock owner 完成后重试。
...
Cocodex 会重新提示 active task，并让 lock 继续属于 Alice，直到 Alice 通过
`cocodex sync` 发布成功。如果存在旧版本遗留状态，`cocodex sync` 和
`cocodex join <name>` 会自动归一化安全状态；不能安全处理时会打印 owner
session 和下一步动作。
...
如果 Cocodex 因没有 tmux target 而拒绝 semantic task，通常说明 `join` 不是从开发者自己的 tmux pane 中启动的。请从该开发者的 tmux pane 重新运行 `cocodex join <name>`，然后重试 `!cocodex sync`。Cocodex 在拒绝前会恢复 snapshot，因此开发者的工作仍然可以重试。

remote sync warning 不会阻断本地开发。之后修复网络或 Git 认证问题即可；Cocodex 会在后续 `cocodex sync` 中重试远程同步。

`integration busy: <name> is syncing ...` 表示另一个 session 正在持有 integration lock 或即将收到 sync task。保持当前 worktree 不变，等对方完成后再次执行 `!cocodex sync`。

`integration busy: <name> is disconnected while syncing ...` 表示 lock owner 在 active sync task 中途断开了。不要绕过这个 lock。该开发者应该在项目仓库重新运行 `cocodex join <name>`，然后从自己的 managed worktree 中运行 `cocodex sync`。其他开发者保持自己的 worktree 不变，等 owner 完成后重试。
...
`cocodex delete refused for alice` 表示这个 session 现在还不能安全删除。
输出会说明具体 blocker：如果有 active task，就让 owner 重新 join 并完成
`cocodex sync`；如果 session 仍 connected，就先关闭旧 `join` 进程；如果有
ignored files，就先检查、归档或清理该 worktree 后再重试 delete。
...
`sync already in progress (publishing)` 应该只是短暂状态。如果它持续存在，重启 daemon，并为受影响的 session 重新运行 `cocodex join <name>`。启动归一化会让可继续的 active task 回到 `fusing`，并在清理未完成 task 启动状态前创建 backup ref。旧 session 缺少 `last_seen_main` 时，Cocodex 只会在 Git ancestry 能证明它只是领先或落后 `main` 时自动采用安全 baseline；真正 divergent 的 unknown-baseline session 会被拒绝并等待 operator 检查。
