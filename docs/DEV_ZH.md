...
本文面向 Cocomerge 项目的维护者，说明当前实现模型和关键状态机。用户使用流程请阅读根目录的 [README.md](../README.md) 或 [中文 README](README_ZH.md)。

## 架构

Cocomerge 是围绕 Git 和 Codex 构建的单机协作编排层，主要由以下部分组成：

- CLI：`src/cocomerge/cli.py`。
- 持久状态：`src/cocomerge/state.py`，使用 `.cocomerge/state.sqlite` 中的 SQLite。
- Daemon 编排：`src/cocomerge/daemon.py`。
- Session 侧协作 agent：`src/cocomerge/agent.py`。
- Session worktree 初始化：`src/cocomerge/session.py`，包括为 Codex 生成 Cocomerge 指导文件和配置 per-worktree Git identity。

daemon 和 session agent 通过 Unix domain socket 传输 JSONL 消息。Git 操作通过 `src/cocomerge/git.py` 中的 helper 调用 Git CLI 完成。

`SessionAgent` 可以把 sync prompt 粘贴到 tmux 中，但只有在 `join` 显式收到 `--tmux-target` 时才会这么做。Cocomerge 刻意不自动识别 `TMUX_PANE`，因为测试、包装脚本和嵌套 shell 可能继承到错误 Codex 的环境变量。收到 `start_fusion` 后，agent 总会在 task file 旁边写出 prompt file 并打印二者路径；如果配置了目标 pane，它才会额外通过 `tmux load-buffer`、`paste-buffer` 和 `send-keys Enter` 注入 prompt。

## 配置模型

`cocomerge init` 通过 `src/cocomerge/config.py` 中的 `init_config()` 写入 `.cocomerge/config.json`。公开配置 schema 包含：

- `main_branch`：Cocomerge 可以发布的本地主分支。
...
- 仅仅落后于 `main` 的 clean session 会被 fast-forward；
- 尚未集成的本地工作会产生“先 review，再开始新工作”的 notice。

`SessionAgent` 会在子命令启动后打印 startup notice。显式配置了 tmux target 时，也会把 notice 粘贴进该 pane。

## 生成的 Session 指导文件

`ensure_session_worktree()` 会在每个 managed worktree 中写入一个 `AGENTS.md`。这个文件告诉 Codex 它正在 Cocomerge session 中工作，并说明正常协作是在该 worktree 中运行 `cocomerge sync`。

这个生成文件不能让 session 一启动就变 dirty。Cocomerge 在写入前会把 `/AGENTS.md` 加入仓库本地的 `.git/info/exclude`，因此 Git status、snapshot 和 `git add -A` 都会忽略它。如果项目本身已经有自己的 `AGENTS.md`，Cocomerge 不会覆盖项目指令。
...
- `branch`：managed session branch，通常是 `cocomerge/<name>`。
...
- `verifying`：Cocomerge 正在验证 candidate。
- `publishing`：Cocomerge 正在移动 `main` 并可选推送 remote。
...
`src/cocomerge/protocol.py` 负责消息结构校验，`src/cocomerge/transport.py` 负责 JSONL socket transport。
...
`process_queue_once()` 只有在 integration lock 空闲时才会启动任务。它会同时认领 lock 和 active task，发送 `freeze`，准备 snapshot，把 snapshot/base ref 保存到 `refs/cocomerge/`，将 session worktree 重置到最新 `main`，写出 task file，然后发送 `start_fusion`。

task file 由 `src/cocomerge/tasks.py` 创建，包含 snapshot commit、latest main、last seen main、diff summary、中断当前开发请求时的处理要求、validation report 要求，以及提交 candidate 后在同一个 worktree 中再次运行 `cocomerge sync` 的指令。
...
本地 publish 后，Cocomerge 会记录 `last_observed_main`、将 session 标记为 clean、释放 lock、fast-forward clean idle sessions，并广播 main update。如果配置了 remote，随后会尝试 best-effort server-ref sync。remote sync 失败不再阻塞本地发布：Cocomerge 会记录 `remote_sync_failed` event，并在后续 `sync` 中重试。
...
- 如果 `main` 被 Cocomerge 以外的过程移动，dirty/queued/active integration session 会进入 `recovery_required`；
- 如果本地 `main` 等于某个持锁 session 的 candidate，且处于 Cocomerge 自己的 pending publish recovery，不会被误判为 external movement。
...
不要发布 `.cocomerge/`、`.pytest_cache/`、`__pycache__/`、内部计划文档或实现测试套件，除非发布策略发生变化。
